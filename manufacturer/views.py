...
def view_quote_bids(request, quote_id):
    ...
    try:
        ...
        for bid in bids:
            bid.supplier_avg_rating = bid.supplier.reviews.all().aggregate(Avg('rating'))['rating__avg'] or 0
        ...
        if sort == 'lowest':
            ...
            bids = sorted(bids, key=lambda x: x.supplier_avg_rating, reverse=True)
